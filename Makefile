...
db: python
	@tox -qe dev --run-command 'python3 -m via.scripts.init_db --create --stamp'
...
fix: python
	@pyenv exec tox -qe lint -- ruff check --fix-only via tests bin

.PHONY: noqa
$(call help,make noqa,"add noqa comments to suppress lint violations")
noqa: python
	@pyenv exec tox -qe lint -- ruff check --add-noqa via tests bin
